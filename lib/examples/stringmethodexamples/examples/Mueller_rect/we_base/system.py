...
class System(WESTSystem):
    ...
    def initialize(self):
        ...
        x = np.append(-float('inf'), np.linspace(-0.8, -0.7, 160))
        x = np.append(x, np.array([-0.6, -0.51]))
        x = np.append(x, np.array([-0.5, -0.15, 40]))
        x = np.append(x, np.array([-0.14, 0.04]))
        x = np.append(x, np.linspace(0.05, 0.15, 4))
        x = np.append(x, np.linspace(0.17, 0.55, 13))
        x = np.append(x, np.array([0.56, 0.67, float('inf')]))

        y = np.linspace(-0.05, 1.6, 24)
        y = np.append(-float('inf'), y)
        y = np.append(y, float('inf'))

        self.bin_mapper = RectilinearBinMapper([x, y])
